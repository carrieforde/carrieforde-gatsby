...
<CallOut
  type="TIP"
  text="If you're using Gatsby markdown without MDX, wrapping the entire template in the merge field component is perfectly feasible."
/>

For this purpose I'll convert my existing `cf-alert` web component into a React component called a `CallOut`.
...
In the body of the component, you can see where the `MergeField` is used. It doesn't matter whether the callout contains a merge field or not, because it will simply return the original text string if a merge field doesn't exist. Let's give this a whirl.

<CallOut text="I make {{adjective}} websites" />
...
If we either forget to pass a UTM parameter, or the API we're using to supply our dynamic data doesn't contain a corresponding value, then we'll end up with a sentence that makes little sense:
...
## Updating our exiting merge field utilities
...
We wrap the `|[\w]+` piece in parentheses to create a capture group, which helps group several different pieces of a regex together (in this case, the `|`, which was escaped with `\|`, by the way), and add a `?` after the closing parenthesis because that entire piece is optional.
...
This regex _looks_ nasty, but the only special piece I added here was `\s`, which looks for whitespace. Everything else are punctuation marks. Our tests should be passing again.

Next, we'll want to update our `extractMergeField()` method to include the default text.
...
This should be the final piece to adding a default value to our merge fields. Let's test it a `CallOut`:

<CallOut text="I use {{technologies|HTML, CSS, and JavaScript}} to build websites." />
...
<CallOut text="I like {{ingredient1|PB}} and {{ingredient2|J}} sandwiches" />

With merge fields, we now have a way of injecting dynamic data into static content, whether it's from a CMS like WordPress, Contentful, etc. or whether you use markdown to manage your site content like I do for this site. In this example, I used query parameters as my data source, but the data just as easily could be retrieved from a rest API.
